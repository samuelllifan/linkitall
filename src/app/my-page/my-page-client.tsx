...
export function MyPageClient({
  ...
  // Music setup dropdown (audio source + settings). The song's metadata and clip
  // trim are edited directly on the player card, not in here.
  ...
  const musicMenuP = usePresence(musicOpen, 200);
  ...
  // Close the music dropdown on Escape or an outside click.
  useEffect(() => {
    if (!musicOpen) return;
    const onKey = (e: KeyboardEvent) => {
    ...
    const onDown = (e: MouseEvent) => {
      if (
        ...
        !musicMenuRef.current.contains(e.target as Node)
      ) {
        setMusicOpen(false);
      }
    };
  ...
  // Live music preview follows the draft while editing, the saved config in view
  // mode. Only shown when enabled.
  const editorMusic = editing ? draft.music : saved.music;
  const activeMusic = editorMusic?.enabled ? editorMusic : undefined;
  ...
  return (
    <>
      ...
      <div className="relative flex w-full flex-1 flex-col">
        ...
        <main className="mx-auto flex min-h-[calc(100vh-3.5rem)] w-full max-w-3xl flex-col items-center justify-center px-6 pt-16 pb-28">
          ...
          >
            ...
            {/* Integrated display: the player sits inside the profile panel —
                editable right here in edit mode so the preview stays accurate. */}
            {activeMusic && activeMusic.display === "integrated" ? (
              <MusicPlayer
                config={activeMusic}
                className="w-full max-w-none"
                editing={editing}
                onChange={(m: MusicConfig) => updateField("music", m)}
              />
            ) : null}
          ...
          {/* The player renders in its true spot so the edit preview is
              accurate: integrated inside the panel (above), own-panel as a card
              below, hidden as the floating mini control. Card modes are editable
              in edit mode. */}
          {activeMusic && activeMusic.display === "panel" ? (
          ...
          {/* Hidden: a small floating play/mute control, bottom-left. */}
          {activeMusic && activeMusic.display === "hidden" ? (
            <MusicPlayer config={activeMusic} variant="mini" />
          ) : null}
        </main>
      </div>
      ...
      {/* Music setup — top-left, under Background, edit mode only. Song details
          and the clip trim are edited on the player card itself. */}
      {editing ? (
        <div ref={musicMenuRef} className="fixed top-32 left-6 z-40">
          <Button
            variant="outline"
          ...
          {musicMenuP.value ? (
            <div
              ...
              aria-label="Music"
              className={cn(
                "absolute top-full left-0 mt-2 max-h-[70vh] w-80 origin-top-left overflow-y-auto rounded-lg border border-border bg-popover p-4 text-popover-foreground shadow-lg",
                musicMenuP.visible ? "animate-pop" : "animate-pop-out",
              )}
            >
              <MusicEditor
                value={draft.music ?? DEFAULT_MUSIC_CONFIG}
                onChange={(m: MusicConfig) => updateField("music", m)}
              />
              {draft.music ? (
                <>
